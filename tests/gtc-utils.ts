...
export function createGTCDistChangedEvent(
  delegator: Address,
  delegatee: Address
): GTCDistChanged {
  let gtcDistChangedEvent = changetype<GTCDistChanged>(newMockEvent())

  gtcDistChangedEvent.parameters = new Array()

  gtcDistChangedEvent.parameters.push(
    new ethereum.EventParam("delegator", ethereum.Value.fromAddress(delegator))
  )
  gtcDistChangedEvent.parameters.push(
